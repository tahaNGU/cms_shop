...
class productController extends Controller
{
    public function index(Request $request,$product_cat=''){
        $product_attribute=[];
        $product_variation=[];
        $products=product::where('is_active','1')->filter()->simplePaginate(12);
        if(isset($product_cat) && !empty($product_cat)){
            ...
            $products=$product_cat->product()->where('is_active','1')->filter()->simplePaginate(12);
        }
        $brands=brand::where('state','1')->get();
        $product_cats=product_cat::where('state','1')->where('parent_id','0')->get();
        return view('site.products',compact('products','brands','product_cats','product_attribute','product_variation','product_cat'));
    }
