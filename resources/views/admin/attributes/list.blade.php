    ...
    <div class="col-lg-12">
        <div class="card">
            @if(isset($attributes[0]))
                <form action="{{route('admin.attribute.action_items')}}" method="post">
                    @csrf
                    <div class="card-body">
                                ...
                                @foreach($attributes as $attribute)
                                    <tr>
                                        <th scope="row"><input type="checkbox" class="check_item" name="check_item[]"
                                                               value="{{$attribute["id"]}}"></th>
                                        <td style="width:230px">{{$attribute["title"]}}</td>
                                        <td style="width:150px" class="d-flex justify-content-center"><input type="text"
                                                                                                             style="width: 20%"
                                                                                                             name="order[{{$attribute["id"]}}]"
                                                                                                             value="{{$attribute['order']}}"/>
                                        </td>
                                        <td style="width:230px">{{__('common.state')[$attribute['state']]}}</td>
                                        <td style="width:150px">{{$attribute->convert_Verta()}}</td>
                                        <td style="width:150px" style="width:150px" class="d-flex">
                                            ...
                                            <a class="btn btn-sm btn-danger"
                                               href="{{route('admin.attribute.delete',['id'=>$attribute["id"]])}}"><i
